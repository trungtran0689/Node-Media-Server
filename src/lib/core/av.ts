...
export const AUDIO_SOUND_RATE = [5512, 11025, 22050, 44100];
...
function getSampleRate(bitop, info: any) {
...
export function getAACProfileName(info) {
...
function HEVCParseSPS(SPS, hevc) {
  ...
  if (psps.conformance_window_flag) {
    const vert_mult = 1 + (psps.chroma_format_idc < 2 ? 1 : 0);
    const horiz_mult = 1 + (psps.chroma_format_idc < 3 ? 1 : 0);
...
export function readAVCSpecificConfig(avcSequenceHeader) {
...
export function getAVCProfileName(info: any): string {
  switch (info.profile) {
    case 1:
      return 'Main';
    case 2:
